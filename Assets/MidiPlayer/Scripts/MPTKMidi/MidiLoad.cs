...
namespace MidiPlayerTK
{
    ...
    /// <summary>
    /// Base class for loading a MIDI file and reading MIDI event in a MIDI sequencer. <b>It is not recommended to instanciate directly this class.</b>\n
    ...
    public partial class MidiLoad
    {
        ...
        /// <summary>
        /// @deprecated MidiLoad.MPTK_BPM2MPQN is deprecated and will be removed. Please investigate MPTKEvent.MPTK_BPM2MPQN.
        /// </summary>
        public static int MPTK_BPM2MPQN(int bpm)
        {
            Debug.LogWarning("MidiLoad.MPTK_BPM2MPQN is deprecated and will be removed. Please investigate MPTKEvent.MPTK_BPM2MPQN in place");
            return 60000000 / bpm;
        }

        /// <summary>
        /// @deprecated MidiLoad.MPTK_MPQN2BPM is deprecated and will be removed. Please investigate MPTKEvent.MPTK_MPQN2BPM.
        /// </summary>
        public static int MPTK_MPQN2BPM(int microsecondsPerQuaterNote)
        {
            Debug.LogWarning("MidiLoad.MPTK_MPQN2BPM is deprecated and will be removed. Please investigate MPTKEvent.MPTK_MPQN2BPM in place");
        ...
        private List<MPTKEvent> ConvertNAudioEventsToMPTKEvents()
        {
            ...
            try
            {
                        ...
                        if (nAudioMidievent.CommandCode == MidiCommandCode.MetaEvent)
                        {
                            ...
                            if (meta.MetaEventType == MetaEventType.SetTempo || meta.MetaEventType == MetaEventType.TimeSignature)
                            {
                                ...
                                else
                                {
                                    // Set default value
                                    double exactTime = 0;
                                    double pulse = (double)MPTKEvent.MPTK_BPM2MPQN(120) / (double)midifile.DeltaTicksPerQuarterNote / 1000d;
                                    int microsecondsPerQuarterNote = MPTKEvent.MPTK_BPM2MPQN(120);
                                    ...
                                    if (meta.MetaEventType == MetaEventType.SetTempo)
                                    {
                                        ...
                                        if (microsecondsPerQuarterNote <= 0)
                                        {
                                            ...
                                            microsecondsPerQuarterNote = MPTKEvent.MPTK_BPM2MPQN((int)120);
                                        }
                                    }
                                    else
                                    {
                ...
                if (MPTK_TempoMap.Count == 0)
                    ...
                    MPTK_TempoMap.Add(new MPTKTempo(index: MPTK_TempoMap.Count, microsecondsPerQuarterNote: MPTKEvent.MPTK_BPM2MPQN(120), pulse: (double)MPTKEvent.MPTK_BPM2MPQN(120) / (double)midifile.DeltaTicksPerQuarterNote / 1000d));
                ...
                if (MPTK_LogLoadEvents)
                    MPTK_TempoMap.ForEach(tempo => Debug.Log($"Tempo change From:{tempo.FromTick} To:{tempo.ToTick} MPQN:{tempo.MicrosecondsPerQuarterNote,10} BPM:{MPTKEvent.MPTK_MPQN2BPM(tempo.MicrosecondsPerQuarterNote),3} Pulse:{tempo.Pulse,2:F2} Cumul:{tempo.FromTime,6:F2}"));
            }
            catch (System.Exception ex)
            {
                MidiPlayerGlobal.ErrorDetail(ex);
            ...
            */
            try
            {
                ...
                foreach (IList<MidiEvent> nAudioTrack in midifile.Events)
                {
                    ...
                    //MPTK_InitialTempo = MPTKEvent.MPTK_MPQN2BPM(MPTK_MicrosecondsPerQuarterNote);
                    //fluid_player_set_midi_tempo(MPTK_MicrosecondsPerQuarterNote);
                    //if (MPTK_LogLoadEvents)
                    //    Debug.Log($"Track {indexLoadingTrack} Initial Index Tempo {indexTempo}, MPQN:{MPTK_MicrosecondsPerQuarterNote} BPM:{MPTKEvent.MPTK_MPQN2BPM(MPTK_MicrosecondsPerQuarterNote)} pulse:{MPTK_Pulse}");
                    foreach (MidiEvent nAudioMidievent in nAudioTrack)
                    {
                        ...
                        //    fluid_player_set_midi_tempo(MPTK_MicrosecondsPerQuarterNote);
                        //    if (MPTK_LogLoadEvents)
                        //        Debug.Log($"New Index Tempo {indexTempo}, MPQN:{MPTK_MicrosecondsPerQuarterNote} BPM:{MPTKEvent.MPTK_MPQN2BPM(MPTK_MicrosecondsPerQuarterNote)} pulse:{MPTK_Pulse}");
                        //}
        ...
        {
            MPTK_TempoMap = new List<MPTKTempo>();
            MPTKTempo.MPTK_CalculateMap(MPTK_DeltaTicksPerQuarterNote, MPTK_MidiEvents, MPTK_TempoMap);
            MPTK_TempoMap = MPTK_TempoMap.OrderBy(o => o.FromTick).ToList();
            ...
            MPTKSignature.MPTK_CalculateMap(MPTK_DeltaTicksPerQuarterNote, MPTK_MidiEvents, MPTK_SignMap);
            MPTKSignature.MPTK_CalculateMeasureBoundaries(MPTK_SignMap);
            ...
            if (MPTK_MidiEvents != null)
            {
                ...
                MPTK_MidiEvents.ForEach(midievent =>
                {
                    ...
                    indexTempo = MPTKTempo.MPTK_FindSegment(MPTK_TempoMap, midievent.Tick, indexTempo);
                    ...
                    indexSign = MPTKSignature.MPTK_FindSegment(MPTK_SignMap, midievent.Tick, indexSign);
                    MPTKSignature signMap = MPTK_SignMap[indexSign];
                    midievent.Measure = signMap.MPTK_TickToMeasure(midievent.Tick);
                    midievent.Beat = signMap.MPTK_CalculateBeat(midievent.Tick, midievent.Measure);
        ...
        public bool calculateTickPlayer(int msec)
        {
            ...
            try
            {
                    ...
                    MPTK_TickPlayer > MPTK_CurrentTempoMap.ToTick)
                {
                    ...
                    MPTK_CurrentIndexTempoMap = MPTKTempo.MPTK_FindSegment(MPTK_TempoMap, MPTK_TickPlayer, MPTK_CurrentIndexTempoMap);
                ...
                {
                    // Normmally, a new index will be found
                    MPTK_CurrentIndexSignMap = MPTKSignature.MPTK_FindSegment(MPTK_SignMap, MPTK_TickPlayer, MPTK_CurrentIndexSignMap);
        ...
        private void fluid_player_set_midi_tempo(int MPQN)
        {
            ...
            else
            {
                ...
                //Debug.Log($"Apres tempo change tempo:{MPQN} BPM:{MPTK_MPQN2BPM(MPQN)} pulse:{MPTK_Pulse} start:{start_msec} ms start:{TickFromTempoChange} ticks");
                start_msec = cur_msec;
