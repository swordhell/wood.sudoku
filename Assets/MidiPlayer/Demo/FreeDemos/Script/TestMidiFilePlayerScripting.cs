...
namespace DemoMPTK
{
    public class TestMidiFilePlayerScripting : MonoBehaviour
    {
        ...
        public bool toggleEnableMetronome;
        ...
        void OnBeatAction(int time, long tick, int measure, int beat)
        {
            if (toggleEnableMetronome)
            {
                ...
                midiFilePlayer.MPTK_PlayDirectEvent(new MPTKEvent() { Command = MPTKCommand.NoteOn, Channel = 9, Value = 60, Velocity = 100, Measure = measure, Beat = beat });
            }
        }
        ...
        void Start()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                //          - accuracy is garantee.
                //          - MIDI Events can be modified before processed by the MIDI synth.
                //          - MIDI Events are skipped if PreProcessMidi return is false v2.10.0
                //          - Direct call to Unity API is not possible.
                ...
                //    - accuracy is garantee.
                //    - Direct call to Unity API is not possible (but you have access to all your script variable).
                midiFilePlayer.OnBeatEvent = (int time, long tick, int measure, int beat) =>
                {
                    if (toggleEnableMetronome)
                    {
                        Debug.Log($"Quarter - time:{time} sec. Tick:{tick} Signature segment:{midiFilePlayer.MPTK_MidiLoaded.MPTK_CurrentSignMap.Index} Measure:{measure} Beat:{beat}");
                        midiFilePlayer.MPTK_PlayDirectEvent(new MPTKEvent() { Command = MPTKCommand.NoteOn, Channel = 9, Value = 60, Velocity = 100, Measure = measure, Beat = beat });
                    }
                };
        ...
        public void MidiReadEvents(List<MPTKEvent> midiEvents)
        {
            ...
            if (StopPositionPct < 100f)
            {
                ...
                if (midiFilePlayer.MPTK_TickCurrent > (long)((float)midiFilePlayer.MPTK_TickLast * (StopPositionPct / 100f)))
                    if (midiFilePlayer.MPTK_MidiAutoRestart)
        ...
        void OnGUI()
        {
            ...
            if (midiFilePlayer != null)
            {
                ...
                GUILayout.Label($"MIDI Start Position: {StartPositionPct} % ({pos})", myStyle.TitleLabel3, GUILayout.Width(300));
                StartPositionPct = (long)GUILayout.HorizontalSlider(StartPositionPct, 0f, 100f, GUILayout.Width(150));
                midiFilePlayer.MPTK_MidiAutoRestart = GUILayout.Toggle(midiFilePlayer.MPTK_MidiAutoRestart, "Automatic MIDI Restart");
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                pos = (long)((float)midiFilePlayer.MPTK_TickLast * (StopPositionPct / 100f));
                GUILayout.Label($"MIDI Stop Position: {StopPositionPct} % ({pos})", myStyle.TitleLabel3, GUILayout.Width(300));
                ...
                if (GUILayout.Button(MidiFilePlayer.ModeStopPlayLabel[(int)ModeLoop]/*, GUILayout.Width(150)*/))
                {
                ...
                GUILayout.EndHorizontal();
                //#if MPTK_PRO
                toggleEnableMetronome = GUILayout.Toggle(toggleEnableMetronome, "  Enable Metronome [Pro]");
