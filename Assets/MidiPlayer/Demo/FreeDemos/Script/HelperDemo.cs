...
namespace DemoMPTK
{
    public class HelperDemo
    {
        ...
        static Color backColor = new Color(0.1f, 0.1f, 0.1f, 1f);
        ...
        static public void DisplayInfoSynth(MidiSynth synth, int width, CustomStyle myStyle)
        {
            ...
            info = $"Mode:\t{modeSynth}\tRate:{synth.OutputRate}\tBuffer:{synth.DspBufferSize}\tDSP:{Math.Round(synth.StatDeltaAudioFilterReadMS, 2),-5:F2} ms\n";
            info += $"Voice:\tPlayed:{ synth.MPTK_StatVoicePlayed,-4}\tFree:{ synth.MPTK_StatVoiceCountFree,-4}\tActive:{synth.MPTK_StatVoiceCountActive,-4}\t";
            info += $"Reused:{Mathf.RoundToInt(synth.MPTK_StatVoiceRatioReused)} %\n";
            ...
            {
                info += string.Format("\nStat Sequencer:\tDeltaTick:{0,-5:F2} ms\tMini:{1,-5:F2}\tMaxi:{2,-5:F2}\tAvg:{3,-5:F2}\n",
                ...
                info += string.Format("\tReadMidi:{0,-5:F2} ms\tTreat:{1,-5:F2}\tMaxi:{2,-5:F2} \tDeltaMidi:{3,-5:F2}",
                       Math.Round(synth.StatReadMidiMS, 2),
                       Math.Round(synth.StatProcessMidiMS, 2),
                       Math.Round(synth.StatProcessMidiMAX, 2),
                       Math.Round(synth.StatDeltaTimeMidi, 2));
            }
